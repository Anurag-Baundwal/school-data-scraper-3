...
async def gemini_based_scraping(url, school_name, nickname, scrape_type):
    user_agents = [
    ...
    try:
        ...
        if scrape_type == 'roster':
            ...
            """
        elif scrape_type == 'coaches':
            prompt = f"""
            Analyze the HTML content of the college softball coaches webpage from {url}. The expected school name is "{school_name}" and the team nickname or name should be related to "{nickname}". Focus ONLY on coach information. Extract the following information for each coach:
            - Name
            - Position/Title
            - Years at School (if available)
            - Alma Mater (if available)
            - Graduation Year from Alma Mater (if available)
            Format the output as a JSON string with the following structure:
            {{
                "success": true/false,
                "reason": "reason for failure" (or null if success),
                "coaches": [
                    {{
                        "name": "...",
                        "position": "...",
                        "yearsAtSchool": "..." (or null if not available),
                        "almaMater": "..." (or null if not available),
                        "graduationYear": YYYY (or null if not available)
                    }},
                    ...
                ]
            }}
            Set "success" to false if:
            1. No coach data is found
            2. Any coach is missing one or more of the required fields (name, position)
            If "success" is false, provide a brief explanation in the "reason" field.
            Important: Ensure all names, including those with non-English characters, are preserved exactly as they appear in the HTML. Do not escape or modify any special characters in names or school names.
            The response should be a valid JSON string only, without any additional formatting or markdown syntax.
            """
        ...
        try:
            ...
            if scrape_type == 'roster':
                ...
                result['coaches_count'] = len(result['coaches'])
                if result['coaches_count'] == 0:
                    result['success'] = False
                    result['reason'] = "No coaches found in the extracted data"
...
async def process_school(school_data, url_column, sheet_name, scrape_type):
    ...
    if pd.notna(url):
        for attempt in range(max_retries):
            try:
                ...
                result, success, input_tokens, output_tokens = await gemini_based_scraping(url, school_name, nickname, scrape_type)
                ...
                if success and result:
                    ...
                    return {
                        ...
                        'success': True,
                        'reason': None,
                        'rosterYear': result.get('rosterYear'),
                        'players': result.get('players', []),
                        'player_count': result.get('player_count', 0),
                        'coaches': result.get('coaches', []),
                        'coaches_count': result.get('coaches_count', 0),
...
async def process_sheet(sheet_name, df):
    ...
    semaphore = asyncio.Semaphore(15)  # Limit to 50 concurrent requests

    async def process_with_semaphore(row, url_column, scrape_type):
        async with semaphore:
            return await process_school(row, url_column, sheet_name, scrape_type)

    for url_column in ['Roster URL', 'Coaches URL']:
        if url_column in df.columns:
            logger.info(f"\nProcessing {url_column} URLs for sheet: {sheet_name}")
            
            scrape_type = 'roster' if 'Roster' in url_column else 'coaches'
            tasks = [process_with_semaphore(row, url_column, scrape_type) for _, row in df.iterrows()]
            ...
            logger.warning(f"No '{url_column}' column found in sheet: {sheet_name}")
